...
<%
G_mb=Request("sk")
G_moban=Request("G_moban")
G_moban_html=Request("G_moban_html")
qingtian_date_v=request("ver") '升级至版本
ps1=request.ServerVariables("URL")
ps1=Mid(ps1,2,len(ps1))
ps1=Replace(ps1,"/update/copyskin.asp","")
ps1_a=Split(ps1,"/")
...
upDir="/backup/晴天升级文件/"&G_mb&"/"&upVerDir&"/"
...
<table border="0" cellspacing="1" cellpadding="3" align=center class="tableBorder"> 
  ...
  <tr> 
    <td class="forumRow">
	<b>正在更新...从<%=qingtian_date_v-1%>到<%=qingtian_date_v%></b>	
	</td> 
  </tr>   
  <tr> 
    <td class="forumRow">
	<div class="dates"> 正在覆盖原文件：<span  id="old_file" style="color:#FF0000">fe</span></div>	
	</td> 
  </tr> 
</table>
<%
...
Function ShowAllFile(Path,Path_1,Tstr)
	...
	For Each myfile in fc2
		IF myfile.name<>"Thumbs.db"  And myfile.name<>qingtian_date_v&".txt" Then
			...
			new_file=Replace(new_file,"template/"&G_mb&"_4_20100527/html/","template/"&G_moban&"/"&G_moban_html&"/")
			new_file=Replace(new_file,"template/"&G_mb&"_4_20100527/","template/"&G_moban&"/")
...
IF N_Fso.C_N_IsFolderExists(Add_Root_Dir(upDir)) Then
	...
	N_Fso.DeleteFolder Add_Root_Dir("/backup/晴天升级文件/"&G_mb&"/")
	N_Fso.DeleteFolder Add_Root_Dir("/backup/晴天备份文件/"&G_mb&"/")
	...
	Response.Write "<meta http-equiv=""Refresh"" content=""1;URL=indexskin.asp?tag=true&s1=0&ver="&qingtian_date_v&""">"
Else
	Response.Write "<font color=red>升级失败：升级文件不存在</font>"
End IF
%>
